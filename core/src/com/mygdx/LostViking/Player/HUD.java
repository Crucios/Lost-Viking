...
public class HUD implements Disposable{
    ...
    private boolean hasGeneratedChooseSkill;
	...
	public HUD(SpriteBatch sb, Player player){
		...
		WarningSkill = new Label("PRESS SPACE TO CHOOSE SKILL", new Label.LabelStyle(font, Color.WHITE));
		WarningSkill.setFontScale(1.75f, 0.5f);
	...
	public void update(float dt) {
		...
		if(player.isChooseSkill() && !hasGeneratedChooseSkill) {
			stage.clear();
			table = new Table();
			
			table.bottom();
			table.setFillParent(true);
			table.add(WarningSkill).expandX().padLeft(350);
			table.row();
			
			//Add HUD Function
			table.add(scoreLabel).expandX().padBottom(27).padLeft(-1050);
			table.add(scoreCountLabel).expandX().padBottom(27).padLeft(-2250);
			
			table.add(hitpointLabel).expandX().padLeft(-700);
			table.add(hitpointCountLabel).expandX().padTop(5).padLeft(-300).padBottom(35);
			stage.addActor(table);
			hasGeneratedChooseSkill = true;
		}
